...
public class AdminCoursePanel extends JPanel {
    ...
    private void onSaveSection() {
        ...
        // 1. Gather Data (UI Thread)
        StringBuilder dayTimeBuilder = new StringBuilder();
        List<String> combinedSlots = new ArrayList<>();

        for (int i = 0; i < dayBoxes.size(); i++) {
            String dayName = (String) dayBoxes.get(i).getSelectedItem();
            String dayAbbr = EditSectionDialog.REVERSE_DAY_MAP.get(dayName);
            String start = startTimeFields.get(i).getText();
            String end = endTimeFields.get(i).getText();

            if (start.isEmpty() || end.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Error: Times for Class " + (i+1) + " cannot be empty.");
                return;
            }
            combinedSlots.add(dayAbbr + " " + start + "-" + end);
        }
        ...
        LoadingDialog loading = new LoadingDialog(owner, "Saving Section...");
        ...
        SwingWorker<String, Void> worker = new SwingWorker<>() {
            ...
            protected String doInBackground() {
                ...
                return adminService.createSection(
                        courseId, instructorId, dayTimeString, roomStr, capacityStr, semesterString, yearStr
                );
            }
            ...
            protected void done() {
                ...
                loading.dispose(); // Close loading dialog
                try {
                    String message = get(); // Get result
                    JOptionPane.showMessageDialog(AdminCoursePanel.this, message);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        // 4. Start
        worker.execute();
        loading.setVisible(true); // Show dialog (blocks until dispose() is called)
    }
